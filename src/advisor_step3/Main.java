package advisor_step3;

import advisor_step3.api.Constants;
import advisor_step3.com.musicAdvisor.dashboard.DashBoard;

/* #Description:
* Now we need to find a way to get a response code from the URL in the user's browser.
* By default, Java doesn't have browser capabilities but has opportunities to create your own simple HTTP server.
*
* #Objectives:
* Using the Spotify authorization guide and the information given here (you need the section Authorization Code Flow),
* improve your program by adding real authorization on Spotify.
...
* Also, in this stage, you should read the Spotify access server point from the command line argument. Server path should be passed to the program using -access argument. If this argument is not set, you should use a default argument, https://accounts.spotify.com.
* Make sure you replace constants to this argument value everywhere!
*
* Output example
...
public class Main {
    public static void main(String[] args) {
        if (args.length > 1 && args[0].equals("-access")) {
            Constants.AUTHORIZATION_SERVER_URL = args[1];
        }
